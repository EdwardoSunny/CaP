...
class RealEnv:
    def __init__(
        ...
        if enable_cameras:
            ...
            logger.info("[RealEnv] Cameras disabled - robot-only mode")
    ...
    def get_obs(self) -> Dict:
        ...
        # Only get camera data if cameras are enabled
        if self.realsense is not None:
        ...
        if self.last_realsense_data:  # Only process camera data if available
            ...
            # If no cameras, use robot timestamp for alignment
            robot_timestamps = last_robot_data["robot_receive_timestamp"]
            last_timestamp = robot_timestamps[-1]
            obs_align_timestamps = last_timestamp - (
                np.arange(self.num_obs_steps)[::-1] * dt
            )
    ...
    def start_episode(self, start_time=None):
        ...
        # Only start camera recording if cameras are enabled
        if self.realsense is not None:
