...
class XArmAPI(object):
    ...
    def set_tgpio_digital(self, ionum, value, delay_sec=None, sync=True):
        ...
        return self._arm.set_tgpio_digital(
            ionum=ionum, value=value, delay_sec=delay_sec, sync=sync
        )
    ...
    def set_vacuum_gripper(
        self, on, wait=False, timeout=3, delay_sec=None, sync=True, hardware_version=1
    ):
        """
    ...
    def write_and_read_holding_registers(
        ...
        :return: tuple((code, regs)) only when code is 0, the returned result is correct.
            code: See the [API Code Documentation](./xarm_api_code.md#api-code) for details.
                Note: code 129~144 means modbus tcp exception, the actual modbus tcp exception code is (code-0x80), refer to [Standard Modbus TCP](../UF_ModbusTCP_Manual.md)
        """
        return self._arm.write_and_read_holding_registers(
            ...
            for i in range(self.axis):
                if (bits >> i) & 0x01:
                    err_info.append(
                        (
                            i + 1,
                            ret[1][i + 1] if is_rad else math.degrees(ret[1][i + 1]),
                        )
                    )
            return 0, err_info
            # ret[1][1] = ret[1][1] if is_rad else math.degrees(ret[1][1])
