...
class ModbusTcpClient(object):
    ...
    def __wait_to_response(
        self, transaction_id=None, unit_id=None, func_code=None, timeout=3
    ):
        ...
        code = -3  # TIMEOUT
        send_transaction_id = (
            transaction_id if transaction_id is not None else self._transaction_id
        )
        ...
        while time.monotonic() < expired:
            ...
            if transaction_id != send_transaction_id:
                logger.warning(
                    "Receive a reply with a mismatched transaction id (S: {}, R: {}), discard it and continue waiting.".format(
                ...
                logger.warning(
                    "Receive a reply with a mismatched protocol id (S: {}, R: {}), discard it and continue waiting.".format(
                ...
                logger.warning(
                    "Receive a reply with a mismatched unit id (S: {}, R: {}), discard it and continue waiting.".format(
                ...
                logger.warning(
                    "Receive a reply with a mismatched func code (S: {}, R: {}), discard it and continue waiting.".format(
                        send_func_code, func_code
                    )
                )
                length = 0
                recv_data = b""
        ...
        if code == 0 and len(recv_data) == 9:
            logger.error(
                "modbus tcp data exception, exp={}, res={}".format(
                    recv_data[8], recv_data
                )
            )
            return recv_data[8], recv_data
        elif code != 0:
            logger.error(
                "recv timeout, len={}, res={}".format(len(recv_data), recv_data)
            )
    ...
    def __pack_to_send(self, pdu_data, unit_id=None):
        ...
        data = struct.pack(
            ">HHHB", self._transaction_id, self._protocol_id, len(pdu_data) + 1, unit_id
        )
        data += pdu_data
        self.sock.send(data)
    ...
    def __read_bits(self, addr, quantity, func_code=0x01):
        ...
        if code == 0 and len(res_data) == 9 + (quantity + 7) // 8:
            return code, [
                (res_data[9 + i // 8] >> (i % 8) & 0x01) for i in range(quantity)
            ]
        else:
    ...
    def __read_registers(self, addr, quantity, func_code=0x03, signed=False):
        ...
        if code == 0 and len(res_data) == 9 + quantity * 2:
            return 0, list(
                struct.unpack(
                    ">{}{}".format(quantity, "h" if signed else "H"), res_data[9:]
                )
            )
        else:
            return code, res_data
    ...
    def write_multiple_coil_bits(self, addr, bits):
        ...
        for i in range(len(bits)):
            if bits[i]:
                datas[i // 8] |= 1 << (i % 8)
        pdu = struct.pack(
            ">BHHB{}B".format(len(datas)), 0x0F, addr, len(bits), len(datas), *datas
        )
        return self.__request(pdu)[0]
    ...
    def write_multiple_holding_registers(self, addr, regs):
        ...
        """
        pdu = struct.pack(
            ">BHHB{}H".format(len(regs)), 0x10, addr, len(regs), len(regs) * 2, *regs
        )
        return self.__request(pdu)[0]
    ...
    def write_and_read_holding_registers(
        self, r_addr, r_quantity, w_addr, w_regs, r_signed=False, w_signed=False
    ):
        """
        func_code: 0x17
        """
        pdu = struct.pack(
            ">BHHHHB{}{}".format(len(w_regs), "h" if w_signed else "H"),
            0x17,
            r_addr,
            r_quantity,
            w_addr,
        ...
        if code == 0 and len(res_data) == 9 + r_quantity * 2:
            return 0, struct.unpack(
                ">{}{}".format(r_quantity, "h" if r_signed else "H"), res_data[9:]
            )
        else:
            return code, res_data
