...
class Gripper(GPIO):
    ...
    def set_gripper_status(self, status, delay_sec=0, sync=True):
        ...
        if code != 0:
            return code
        if status:
            code1 = self.set_tgpio_digital(
                ionum=0, value=1, delay_sec=delay_sec, sync=sync
            )
            code2 = self.set_tgpio_digital(
                ionum=1, value=0, delay_sec=delay_sec, sync=sync
            )
        else:
            code1 = self.set_tgpio_digital(
                ionum=0, value=0, delay_sec=delay_sec, sync=sync
            )
            code2 = self.set_tgpio_digital(
                ionum=1, value=1, delay_sec=delay_sec, sync=sync
            )
