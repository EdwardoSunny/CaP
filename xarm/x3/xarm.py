...
class XArm(Gripper, Servo, Record, RobotIQ, BaseBoard, Track, FtSensor, ModbusTcp):
    ...
    def _is_out_of_joint_range(self, angle, i):
        if (
            ...
            return False
        device_type = (
            int("{}1305".format(self.axis))
            if self.sn and int(self.sn[2:6]) >= 1305 and int(self.sn[2:6]) < 8500
    ...
    def _set_position_absolute(
        ...
        **kwargs,
        ...
        tcp_pos = [
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[0])
                if x is None
                else float(x)
            ),
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[1])
                if y is None
                else float(y)
            ),
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[2])
                if z is None
                else float(z)
            ),
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[3])
                if roll is None
                else to_radian(roll, is_radian)
            ),
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[4])
                if pitch is None
                else to_radian(pitch, is_radian)
            ),
            (
                (math.inf if self.version_is_ge(2, 4, 101) else self._last_position[5])
                if yaw is None
                else to_radian(yaw, is_radian)
            ),
        ]
        ...
        trans_id = self._get_feedback_transid(
            feedback_key, studio_wait, kwargs.get("is_pop", True)
        )
    ...
    def run_blockly_app(self, path, **kwargs):
        ...
        try:
            if not os.path.exists(path):
                dir_name = (
                    "lite6"
                    if self.axis == 6 and self.device_type == 9
                    else (
                        "850"
                        if self.axis == 6 and self.device_type == 12
                        else (
                            "xarm7T"
                            if self.axis == 7 and self.device_type == 13
                            else "xarm{}".format(self.axis)
                        )
                    )
                )
                path = os.path.join(
                    ...
                    for _ in range(times):
                        exec(
                            blockly_tool.codes,
                            {
                                ...
                                "print": blockly_print,
                                "run_blockly": blockly_exec,
                                "start_run_blockly": blockly_exec,
                                "start_run_gcode": blockly_run_gcode,
                            },
                        )
                except Exception as e:
    ...
    def iden_joint_friction(self, sn=None):
        ...
        valid_xarm7t = (
            not self.is_850 and not self.is_lite6 and sn[0] == "C" and sn[1] == "S"
        )
        valid_xarm = (
            not self.is_850
