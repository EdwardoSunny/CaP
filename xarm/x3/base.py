...
class Base(BaseObject, Events):
    ...
    def _handle_report_data(self, data):
        ...
        def __handle_report_rich(rx_data):
            ...
            if length >= 252:
                temperatures = list(
                    struct.unpack(">7b", struct.pack(">7B", *rx_data[245:252]))
                )
    ...
    def _get_feedback_transid(self, feedback_key, studio_wait=False, is_pop=True):
        return (
            (
                self._fb_key_transid_map.pop(feedback_key, -1)
                if is_pop
                else self._fb_key_transid_map.get(feedback_key, -1)
            )
            if not studio_wait
            else -1
        )
    ...
    def set_control_modbus_baudrate(self, baud):
        code = self.checkset_modbus_baud(
            baud, check=False, host_id=XCONF.LINEER_TRACK_HOST_ID
        )
        self.log_api_info(
            "API -> set_control_modbus_baudrate -> code={}".format(code), code=code
        )
        return code
