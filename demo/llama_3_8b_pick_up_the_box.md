# pick up the box and place the battery inside it

| Field | Value |
|---|---|
| **Model** | `dganochenko/llama-3-8b-chat` |
| **Context (objects)** | box, battery, plastic, tin foil |
| **Generated at** | 2026-04-15 06:10:33 |

## Generated Code

```python
goto_pos(get_obj_pos('box'))
open_gripper()
wait(0.5)
goto_pos(get_obj_pos('battery'))
pick_place(get_obj_pos('battery'), get_obj_pos('box'))
close_gripper()
wait(1.0)
```
...
#
# IMPORTANT API NOTES:
# - get_obj_pos(object_name) returns [x, y, z, roll, pitch, yaw] in MILLIMETERS and DEGREES
#   This includes the optimal grasp pose from GraspGen (position + orientation)
# - goto_pos(pose) accepts either:
#   * [x, y, z] - moves to position, default top-down orientation
#   * [x, y, z, roll, pitch, yaw] - moves to position AND rotates to orientation
# - All distances are in MILLIMETERS (100mm = 10cm = 0.1m)
# - move_up(distance) and move_down(distance) take distance in MILLIMETERS
# - Positions are in robot base frame (origin at robot base)
# - COORDINATE CONVENTION:
#   * +X = forward (away from robot), -X = backward (toward robot)
#   * +Y = left,                      -Y = right
#   * +Z = up,                        -Z = down
# - NEVER add code to return to a neutral/home position. Only generate code to complete the task.
#
import numpy as np
...
# move the arm up by 10cm
say('Raising the arm up by 10 centimeters')
move_up(distance=100)  # distance in millimeters (100mm = 10cm)

# move the arm down by 10cm
say('Lowering the arm down by 10 centimeters')
move_down(distance=100)  # distance in millimeters (100mm = 10cm)

objects = ['orange disinfecting wipes', 'cup', 'bottle']
# move to the orange disinfecting wipes
say('Moving to the orange disinfecting wipes')
wipes_pose = get_obj_pos('orange disinfecting wipes')  # Returns [x, y, z, roll, pitch, yaw] in mm and degrees
goto_pos(wipes_pose)  # goto_pos accepts 6DOF pose and uses the orientation for proper grasping

objects = ['red cup', 'blue bottle', 'green marker']
# go to the cup with proper grasp orientation
say('Going to the red cup with grasp orientation')
cup_pose = get_obj_pos('red cup')  # GraspGen provides optimal grasp pose
goto_pos(cup_pose)  # Moves to position AND rotates to grasp orientation

objects = ['keyboard', 'mouse', 'monitor']
# move above the keyboard
say('Moving 10cm above the keyboard')
keyboard_pose = get_obj_pos('keyboard')
# Modify Z position to be 100mm (10cm) higher, keep orientation
above_keyboard = [keyboard_pose[0], keyboard_pose[1], keyboard_pose[2] + 100, keyboard_pose[3], keyboard_pose[4], keyboard_pose[5]]
goto_pos(above_keyboard)

objects = ['orange disinfecting wipes', 'blue towel', 'green spray bottle']
# detect and pick up the wipes
say('Detecting and picking up the orange disinfecting wipes')
wipes_pose = get_obj_pos('orange disinfecting wipes')  # Returns optimal grasp pose from GraspGen
# Move above the object first (approach from above)
approach_pose = [wipes_pose[0], wipes_pose[1], wipes_pose[2] + 100, wipes_pose[3], wipes_pose[4], wipes_pose[5]]
goto_pos(approach_pose)
wait(0.5)
# Open gripper
open_gripper()
wait(0.5)
# Move down to grasp position with correct orientation
goto_pos(wipes_pose)
wait(0.5)
...
wait(1.0)
# Move up with object
move_up(distance=100)  # Move up 100mm

objects = ['bottle', 'box', 'tape']
# go to the bottle and move 10cm to the right
say('Going to the bottle and moving 10cm right')
bottle_pose = get_obj_pos('bottle')
goto_pos(bottle_pose)
wait(0.5)
# Move 100mm (10cm) right (-Y direction) while keeping orientation
right_of_bottle = [bottle_pose[0], bottle_pose[1] - 100, bottle_pose[2], bottle_pose[3], bottle_pose[4], bottle_pose[5]]
goto_pos(right_of_bottle)

objects = ['red cup', 'blue plate', 'spoon']
# pick up the red cup and place it on the blue plate
say('Picking up the red cup and placing it on the blue plate')
pick_pos = get_obj_pos('red cup')
place_pos = get_obj_pos('blue plate')
pick_place(pick_pos, place_pos)

objects = ['screwdriver', 'toolbox', 'wrench']
# put the screwdriver in the toolbox
say('Putting the screwdriver in the toolbox')
pick_pos = get_obj_pos('screwdriver')
place_pos = get_obj_pos('toolbox')
pick_place(pick_pos, place_pos)

objects = ['banana', 'apple', 'bowl']
# move the banana to the bowl
say('Moving the banana to the bowl')
pick_pos = get_obj_pos('banana')
place_pos = get_obj_pos('bowl')
pick_place(pick_pos, place_pos)
box, battery, plastic, tin foil
# Query: pick up the box and place the battery inside it.
```
